...
\pagebreak
    ...
    \begin{figure}[H]
    \centering
        \includegraphics[width=1.0\textwidth, keepaspectratio=true]{MH161174__all_reads.pdf}
        \caption{Coverage depth of the example dataset (\textit{Nuphar japonica} NC_045072) as generated via \texttt{PACVr.complete()} when no filtering is conducted during read indexing via samtools.}
    \centering
    \end{figure}
...
\section{Output under different samtools filtering options}

    \begin{figure}[H]
    \centering
        \includegraphics[width=0.5\textwidth, keepaspectratio=true]{MH161174__map_to_one_location_only.pdf}
    ...
    \begin{figure}[H]
    \centering
        \includegraphics[width=0.5\textwidth, keepaspectratio=true]{MH161174__map_to_multiple_locations.pdf}
