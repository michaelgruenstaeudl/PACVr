...
ExtractAllRegions <- function(gbkData) {
  ...
  allRegions <- BiocGenerics::as.data.frame(genbankr::otherFeatures(gbkData))
  regions <- tryCatch(tryCatch(filter(allRegions,"note"), warning = function(w) filter(allRegions,"standard_name")), 
                       error = function(e) return(data.frame(start=c(-1),end=c(-1),note=c("empty"),stringsAsFactors = FALSE)))
...
fillDataFrame <- function(gbkData,regions){
  ...
  if((nrow(regions) == 0) || (regions[1,2] == -1)){
    ...
    if(regionAvail == 5) { # only IRa and IRb 
    ...
    }else if(regionAvail == 7){ # only IRa, SSC and IRb
      regions[which(regions[,4] != "NA"), 6] <- 0
      regions[which(regions[,6] == max(regions[,6])),4] <- "LSC"
      message("Annotation for LSC was automatically added")
    }else if(regionAvail == 10){ # only LSC and SSC
    ...
    }else if(regionAvail == 11){ # only LSC, SSC and IRa
    ...
    }else if(regionAvail == 13){ # only LSC, IRb and IRa
      regions[which(regions[,4] != "NA"), 6] <- 0
      regions[which(regions[,6] == max(regions[,6])),4] <- "SSC"
      message("Annotation for SSC was automatically added")
    }else if(regionAvail == 14){ # only LSC, IRb and SSC
